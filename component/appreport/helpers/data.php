...
class Component_Appreport_Helpers_Data extends appRain_Base_Objects
{
	...
	public function ReportCategories(){
		$categories = App::Model('Category')->findAll("type='appreportgroup'");
		return $categories['data'];
	}
	
	public function ReportByGroup($gid=null){
		return App::Model('AppReportCode')->findAll("groups LIKE '%{$gid}%'");
	}
	...
	public function Report($id=null, $field=null){		
		if(isset($id) || isset($field)){
			...
			if(isset($field)){
				if($field == 'code'){
					if(!empty($data[$field])){
						$data[$field] = isset($data[$field]) ?  App::Utility()->fatchfilecontent($this->getFilePath($data[$field])) : '';
					}
				}
