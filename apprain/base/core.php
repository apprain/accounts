...
class appRain_Base_Core extends appRain_Collection
{
    ...
    public function veryfirst_render()
    {
        ...
        if ($config['cookie_domaint'] == '.localhost') $config['cookie_domaint'] = NULL;
    ...
    public function render($render_path = null)
    {
        ...
        if (
            ...
            if (isset($COData['controller_path'])) {
        ...
        if (!(strtolower($this->layout) == appRain_Base_Core::ADMIN_VIEW_LAYOUT_NAME
            && file_exists($layout_path = VIEW_PATH . DS . appRain_Base_Core::ADMIN_VIEW_NAME . DS . appRain_Base_Core::LAYOUT_DIR_NAME . DS . $this->layout . TPL_EXT))
        ) {
    ...
    public function callElement($element_path = NULL, $args = NULL)
    {
        ...
        if (
            !(strtolower($this->layout) == appRain_Base_Core::ADMIN_VIEW_LAYOUT_NAME
                && file_exists($element_render_path = VIEW_PATH . DS . appRain_Base_Core::ADMIN_VIEW_NAME . DS . "elements" . DS . $element_path . TPL_EXT))
        ) {
                ...
                if (($class = $node['resource'][0]) != "" && ($method = $node['resource'][1]) != "") {
                    ...
                    if (!empty($_rtn_resources)) {
                        foreach ($_rtn_resources as $res) {
                            if (isset($name)) {
                                if (strtolower($name) == strtolower($res['name'])) return $res['model_path'];
                            }
                            else {
    ...
    public function fetchAddonlibs()
    {
        ...
        foreach ($sys_addons as $addon_name => $addon) {
            if (
                ($addon['status'] == 'Active')
                && (strtolower($addon['layouts']) == 'all'
                || in_array($this->layout, explode(',', strtolower($addon['layouts']))))
                ...
                if ($addon['load'] == 'Always' || in_array($addon_name, $this->addons)) {
                    ...
                    if (isset($addon['javascripts'])) {
                        foreach ($addon['javascripts'] as $src) echo App::Load("Helper/Html")->add_javascript($src);
                    }

                    if (isset($addon['style_sheets'])) {
                        foreach ($addon['style_sheets'] as $link) echo App::Load("Helper/Html")->add_css($link);
                    }
